...
{
    public AgentServiceRuntimeInfo GetRuntimeInfo(ServiceDescriptor service, string? previousVersion)
    {
        ...
        if (pid is not null)
        {
            try
            {
                ...
                if (!process.HasExited)
                {
                    ...
                    uptimeSeconds = Math.Max(0, (long)(DateTimeOffset.Now - startedAt.Value).TotalSeconds);
                }
            }
        ...
        var dataDirectory = Directory.GetParent(service.RuntimeDirectory)?.FullName ?? service.RuntimeDirectory;
        return new AgentServiceRuntimeInfo(
            pid,
            startedAt,
            uptimeSeconds,
