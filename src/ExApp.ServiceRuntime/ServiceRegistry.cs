...
public sealed class ServiceRegistry(PackageManager packageManager, ServiceRuntimeOptions options)
{
    ...
    public PackageState? GetPackageState(string serviceId) => packageManager.GetState(serviceId);
    ...
    private string? ResolveMockServiceExecutable()
    {
        ...
        {
            var candidate = Path.Combine(current.FullName, "services", "MockService", "bin", "Debug", "net8.0", "ExApp.Service.MockService.exe");
