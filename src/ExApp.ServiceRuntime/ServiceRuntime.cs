    ...
    ServiceLogRouter logs)
{
    ...
    public async Task<IReadOnlyList<AgentServiceInfo>> ListAsync(CancellationToken cancellationToken = default)
    {
        ...
        foreach (var service in registry.List())
        {
                ...
                status)
            {
                ...
                Runtime = monitor.GetRuntimeInfo(
                    service,
                    registry.GetPackageState(service.ServiceId)?.PreviousVersion)
            };
            results.Add(info);
        }
    ...
    public async Task<AgentServiceStatus> RestartAsync(string serviceId, CancellationToken cancellationToken = default)
    {
        await StopAsync(serviceId, cancellationToken);
