...
- [x] DONE — проверить catalog schema
- [x] DONE — подписать catalog
- [ ] TODO — опубликовать catalog

MVP workflow: `.github/workflows/release-services.yml` builds `mock-service-0.1.0-win-x64.svcpkg`, writes hash/size sidecars, generates `artifacts/services.stable.json` with the GitHub Release asset URL, and uploads the package, release catalog and schema to a GitHub Release when a `services-v*` tag is pushed or the workflow is run manually.

The repository `catalog/services.stable.json` stays development-friendly and points to the local `artifacts` package. The published release catalog is generated by the workflow and should be used by clients through `EXAPP_CATALOG_URL`.
